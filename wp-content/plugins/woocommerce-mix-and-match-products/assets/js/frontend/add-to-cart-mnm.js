 ...
 */
function wc_mnm_price_format( price ) {
	return wc_mnm_woocommerce_number_format( wc_mnm_number_format( price ) );
}
...
( function( $ ) {
	...
	function WC_MNM_Container( data ) {
		...
		this.update_addons_totals = function( triggered_by ) {
			...
			if( typeof( triggered_by ) === 'undefined' ) {
				...
				container.$mnm_form.trigger( 'woocommerce-product-addons-update' );
				return false;
			}
	...
	function WC_MNM_Child_Item( container, $mnm_item, index ) {
		...
		this.reset = function() {
			...
			switch( type ) {
				case 'checkbox':
					original_value = this.$mnm_item_qty.is( '[checked="checked"]' );
					this.$mnm_item_qty.prop( 'checked', original_value );
				break;
				case 'select':
					original_value = this.$mnm_item_qty.children( '[selected="selected"]' ).val();
					original_value = 'undefined' !== typeof original_value ? original_value : this.$mnm_item_qty.children( ':first-child' ).val();
					this.$mnm_item_qty.val( original_value );
				break;
				default:
					// Need to use pure JS to get original value from attribute.
					original_value = this.$mnm_item_qty.get(0).getAttribute( 'value' );
					original_value = original_value !== '' ? parseFloat( original_value ) : '';
					this.$mnm_item_qty.val( original_value );
			}
		};
