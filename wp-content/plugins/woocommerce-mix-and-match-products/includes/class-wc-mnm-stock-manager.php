			...
			try {
				...
				if ( $managed_product->is_sold_individually() ) {
					...
					if ( ( ! $updating_cart && $quantity_in_container > 1 ) ) {
						$notice = sprintf( __( '&quot;%s&quot; cannot be added to your cart as configued. %s', 'woocommerce-mix-and-match-products' ), $container_title, $reason );
						throw new Exception( $notice );
					} else if ( $updating_cart && ( $quantity_in_cart + $quantity_in_container ) > 1 ) {
						$notice = sprintf( __( '&quot;%s&quot; cannot be purchased as configured. %s', 'woocommerce-mix-and-match-products' ), $container_title, $reason );
						throw new Exception( $notice );
					}
				}
				...
				if ( ! $managed_product->is_in_stock() ) {
					...
					// Not enough stock for this configuration.
						...
						$reason = sprintf( __( 'There is not enough stock of &quot;%s&quot; (%s remaining).', 'woocommerce-mix-and-match-products' ), $managed_product_title, $managed_product->get_stock_quantity() );

						if ( $updating_cart ) {
							$notice = sprintf( __( '&quot;%s&quot; cannot be purchased as configured. %s', 'woocommerce-mix-and-match-products' ), $container_title, $reason );
						} else {
							$notice = sprintf( __( '&quot;%s&quot; cannot be added to your cart as configured. %s', 'woocommerce-mix-and-match-products' ), $container_title, $reason );
						}
				...
				if ( $managed_product->managing_stock() && $quantity_in_cart > 0 && ! $managed_product->has_enough_stock( $quantity_in_cart + $quantity_in_container ) ) {
					...
					if ( $updating_cart ) {
						...
						$notice = sprintf( __( '&quot;%s&quot; cannot  be added to the cart as configured. %s', 'woocommerce-mix-and-match-products' ), $managed_product_title, $reason );
					}
